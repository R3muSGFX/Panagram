...
class Panagram  {
    ...
    func interactiveMode() {
        ...
        while !shouldQuit {
            ...
            case .palindrome:
                consoleIO.writeMessage("Type a word or a sentence:")
    ...
    func staticMode() {
        ...
        case .anagram:
            if argCount != 4 {
                ...
                if (first.isAnagramOf(second)) {
                    consoleIO.writeMessage("\(second) is anagram of \(first)")
                } else {
                    consoleIO.writeMessage("\(second) is anagram of \(first)")
                }
            }
        case .palindrome:
            if (argCount != 3) {
                if argCount > 3 {
                    consoleIO.writeMessage("Too many arguments for option \(option.rawValue)")
                } else {
                    consoleIO.writeMessage("Too few arguments for option \(option.rawValue)")
                }
                consoleIO.printUsage()
            } else {
